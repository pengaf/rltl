...
class DiscreteBoxToIndex
{};

template<>
class DiscreteBoxToIndex<NumArray>
{
public:
	typedef NumArray Element_t;
	typedef VectorSpace<Element_t> BoxSpace_t;
	typedef paf::SharedPtr<BoxSpace_t> BoxSpacePtr;
public:
	DiscreteBoxToIndex(BoxSpacePtr boxSpace) :
		m_low(boxSpace->low()),
		m_high(boxSpace->high())
	{}
public:
	size_t elementToIndex(const Element_t& element) const
	{
		return 0;
	}
	size_t count() const
...
template<typename Scalar_t, size_t t_size_0, size_t... t_sizes>
class DiscreteBoxToIndex<Array<Scalar_t, t_size_0, t_sizes...>>
{
public:
	typedef Array<Scalar_t, t_size_0, t_sizes...> Element_t;
	typedef VectorSpace<Element_t> BoxSpace_t;
	typedef paf::SharedPtr<BoxSpace_t> BoxSpacePtr;
public:
	DiscreteBoxToIndex(BoxSpacePtr boxSpace) :
...
//{
//public:
//	typedef VectorSpace<t_src_dimension, SrcReal_t> SrcSpace_t;
//	typedef NormalizedDiscreteSpace<DstInteger_t> DstSpace_t;
//	typedef typename SrcSpace_t::Element_t SrcElement_t;
//	typedef typename DstSpace_t::Element_t DstElement_t;
//private:
